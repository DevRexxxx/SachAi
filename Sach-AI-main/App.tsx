...
const App: React.FC = () => {
  ...
  const handleFramesExtracted = useCallback(async (extractedFrames: VideoFrame[]) => {
    ...
    try {
      ...
      if (intervalRef.current) clearInterval(intervalRef.current);
      setProgress(100); setAnalysis(result as AgentResult); setStatus(AppStatus.COMPLETED);
    } catch (err: any) {
      if (intervalRef.current) clearInterval(intervalRef.current);
      setError(err?.message || "Scanner Failure"); setStatus(AppStatus.ERROR);
    }
  }, []);
              ...
              {analysis && (
                <div className={`glass-card p-6 rounded-3xl border ${theme.border} space-y-6 animate-in slide-in-from-left-4`}>
                  ...
                  <button
                    ...
                    className={`group relative w-full py-4 px-6 mt-4 flex items-center justify-center gap-3 
