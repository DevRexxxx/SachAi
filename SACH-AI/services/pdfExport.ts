...
export const exportToPDF = async (analysis: AgentResult, fileName = 'SachAI_Forensic_Report.pdf', mediaDataUrl?: string): Promise<void> => {
  ...
  // ── Verdict banner & Thumbnail ──
  let bannerWidth = contentWidth;
  if (mediaDataUrl) {
    bannerWidth = contentWidth - 55;
    try {
      doc.addImage(mediaDataUrl, margin + bannerWidth + 5, y - 5, 50, 35);
    } catch (e) {
  ...
  doc.roundedRect(margin, y, bannerWidth, 22, 3, 3, 'FD');
  ...
  y = addSectionTitle(doc, 'TECHNICAL CONFIDENCE METRICS', margin, y, verdictColor);
  
  const drawBar = (label: string, score: number, yPos: number, barColor: [number, number, number]) => {
    doc.setFontSize(9);
    doc.setTextColor(100, 100, 120);
    doc.text(label, margin, yPos);
    doc.setFillColor(230, 230, 240);
    doc.rect(margin + 40, yPos - 3, 100, 4, 'F');
    doc.setFillColor(...barColor);
    doc.rect(margin + 40, yPos - 3, score, 4, 'F');
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(20, 20, 30);
    doc.text(`${Math.round(score)}%`, margin + 145, yPos);
    doc.setFont('helvetica', 'normal');
  };

  drawBar('Overall Integrity', analysis.integrityScore, y + 5, verdictColor);
  let metricsY = y + 15;

  let faceScore: number | null = null;
  let fullScore: number | null = null;
  analysis.forensicInsights?.forEach(insight => {
    if (insight.includes('Face-crop score:')) {
      faceScore = parseFloat(insight.split(':')[1]) * 100;
    }
    if (insight.includes('Full-image score:')) {
      fullScore = parseFloat(insight.split(':')[1]) * 100;
    }
  });

  if (faceScore !== null) {
    drawBar('Face Authenticity', faceScore, metricsY, faceScore > 80 ? [5, 150, 105] : faceScore > 40 ? [202, 138, 4] : [220, 38, 38]);
    metricsY += 10;
  }
  if (fullScore !== null) {
    drawBar('Bg Authenticity', fullScore, metricsY, fullScore > 80 ? [5, 150, 105] : fullScore > 40 ? [202, 138, 4] : [220, 38, 38]);
    metricsY += 10;
  }
  y = metricsY + 5;
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.text('SachAI – Neural Forensics Unit  |  SachAI Core', margin, pageHeight - 5);
