...
const App: React.FC = () => {
  ...
  const handleExportPDF = useCallback(async () => {
    if (!analysis) return;
    const mediaImage = frames.length > 0 ? frames[0].dataUrl : undefined;
    await exportToPDF(analysis, 'SachAI_Forensic_Report.pdf', mediaImage);
  }, [analysis, frames]);
        ...
        <div className="flex-grow flex flex-col items-center justify-center p-6 text-white overflow-hidden relative">
          ...
          <div className="relative z-10 text-center space-y-8 animate-in fade-in zoom-in duration-1000 max-w-4xl">
            ...
            <div className="relative flex items-start justify-center gap-4">
              ...
              <span className="text-xl md:text-3xl font-bold text-violet-400 mt-4 md:mt-12 bg-violet-500/10 px-3 py-1 rounded-lg border border-violet-500/20 backdrop-blur-sm">V5.0</span>
            </div>
          ...
          <nav className="glass-card border-b border-white/5 px-6 py-4 sticky top-0 z-50">
            <div className="max-w-[1800px] mx-auto flex flex-wrap justify-between items-center gap-4">
              <div className="cursor-pointer flex items-baseline gap-3" onClick={() => setIsLanding(true)}>
                ...
                <span className="text-[10px] md:text-xs font-bold text-violet-400 bg-violet-500/10 px-1.5 py-0.5 rounded border border-violet-500/20">V5.0</span>
              ...
              <div className={`glass-card p-6 rounded-3xl border ${theme.border}`}>
                ...
                {status === AppStatus.IDLE ? (
                  ...
                  <div className="space-y-6">
                    <div className={`rounded-xl overflow-hidden border border-white/10 relative bg-black/40 ${status === AppStatus.ANALYZING ? 'scanning-overlay' : ''} flex justify-center p-2`}>
                      <div className="relative inline-block max-w-full">
                        <img 
                          src={frames[0]?.dataUrl ?? ''} 
                          className={`transition-all duration-500 ${!mediaRevealed && analysis?.isExplicit ? 'blur-xl scale-110 opacity-50' : ''}`} 
                          style={{ display: 'block', maxWidth: '100%', maxHeight: '600px', width: 'auto', height: 'auto' }}
                          alt="Evidence" 
                        />
                        {!mediaRevealed && analysis?.isExplicit && (
                          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/50 p-4 text-center z-20">
                            ...
                            <h4 className="text-white font-bold mb-2">Explicit Content Detected</h4>
                            <p className="text-zinc-300 text-xs mb-4">This media has been blurred for your safety.</p>
                            <button onClick={() => setMediaRevealed(true)} className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white text-xs font-bold rounded-lg transition-colors">Reveal Media</button>
                          </div>
                        )}
                        {(mediaRevealed || !analysis?.isExplicit) && (
                          <div className="absolute inset-0 pointer-events-none z-10 overflow-hidden">
                            {analysis?.anomalies?.map((anomaly, idx) => {
                              if (!anomaly.box || anomaly.box.length !== 4) return null;
                              const [ymin, xmin, ymax, xmax] = anomaly.box;
                              return (
                                <div
                                  key={idx}
                                ...
                                >
                                  <span className="absolute -top-5 left-0 bg-red-500 text-white text-[8px] font-bold px-1 whitespace-nowrap">
                                    {anomaly.description}
                                  </span>
                                </div>
                              );
              ...
              {analysis && (
                <div className={`glass-card p-6 rounded-3xl border ${theme.border} space-y-6 animate-in slide-in-from-left-4`}>
                  ...
                  <button
                    onClick={() => {
                      if (analysisHistory.length < 2) {
                        showToast(t('counter.needTwoAnalyses', 'Need at least 2 analyses to compare.'));
                        return;
                      }
                    ...
                    className={`group relative w-full py-3 px-6 flex items-center justify-center gap-3 
                      ...
                      transition-all duration-300 hover:shadow-[0_0_20px_rgba(139,92,246,0.15)] 
                      active:scale-[0.98] ${analysisHistory.length < 2 ? 'opacity-50' : ''}`}
                  >
                    <div className="absolute inset-0 bg-violet-500 opacity-0 group-hover:opacity-[0.05] transition-opacity duration-300" />
                    <svg className={`w-4 h-4 ${analysisHistory.length < 2 ? 'text-zinc-500' : 'text-violet-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span className={`text-[11px] font-black uppercase tracking-[0.25em] transition-colors ${analysisHistory.length < 2 ? 'text-zinc-500' : 'text-violet-300 group-hover:text-violet-200'}`}>
                      {analysisHistory.length < 2 ? t('counter.compareDisabled', 'COMPARE (NEEDS 2+ SCANS)') : t('counter.compare', 'COMPARE')}
                    </span>
                  </button>
                  {/* New Scan */}
                  <button
                  ...
                  >
                    ...
                    <span className="text-[11px] font-black uppercase tracking-[0.25em] text-white">NEW SCAN</span>
                  </button>
                </div>
              )}
            </div>
                  ...
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 relative z-10">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-12 border-t border-white/5 pt-10">
                      <div className="space-y-6">
                        ...
                        <p className="text-lg md:text-xl font-light leading-relaxed opacity-90 text-zinc-300" dangerouslySetInnerHTML={{ __html: analysis.explanation }} />
                        <div className="pt-10 space-y-6">
                          ...
                          <div className="map-glow h-64 w-full border border-white/5 flex items-center justify-center relative bg-black/40">
                            ...
                            <span className="text-[9px] font-mono opacity-40 uppercase tracking-[0.5em] text-center px-4 text-emerald-400">
                              {analysis.circulationChannels?.includes('Unknown') ? 'NO SIGNIFICANT SPREAD DETECTED' : 'CIRCULATION NODES IDENTIFIED'}
                            </span>
                          </div>
                        </div>
                      </div>
